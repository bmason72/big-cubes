...
def main():
    ...
    # (now also showing projected WSU cube size, NSPW, Ntargets)
    # -------------------------------------------------
    mous_sorted = sorted(
        mous_100,
    ...
    print("*** Top 10 worst offending MOUS ***")
    print(
        f"{'MOUSuid':>30}  "
        f"{'CycleOfOrigin':>8}  "
        f"{'ProdSize[TB]':>12}  "
    ...
    for m in mous_sorted[:10]:
        # rows for this MOUS + cycle
        mask = ((t['mous'].astype(str) == m['mous']) &
                (t['cycle_info'].astype(str) == m['cycle_info']))
        rows = t[mask]

        if len(rows) == 0:
            cube_gb = float('nan')
            nspw_proj = float('nan')
            ntargets_proj = float('nan')
        else:
            # Representative stats per MOUS:
            # - cube size: mean of wsu_cubesize_initial_stepped2 (in GB)
            # - NSPW and Ntargets: max across matching rows (robust if duplicated)
            cube_gb = np.mean(rows['wsu_cubesize_initial_stepped2'].to(u.Gbyte)).value
            nspw_proj = float(np.max(np.asarray(rows['wsu_nspw_initial'], dtype=float)))
            ntargets_proj = float(np.max(np.asarray(rows['ntarget'], dtype=float)))

        tb_cap_mous = gb_to_tb(m['prodsize_GB_capped'])

        print(
            f"{m['mous']:>30}  {m['cycle_info']:>8}  "
            f"{tb_cap_mous:12.2f}  {cube_gb:10.2f}  "
            f"{nspw_proj:5.0f}  {ntargets_proj:9.0f}"
        )
